...
knitr::include_graphics("/images/terminal.png", error = FALSE)
```
...
Table: Equivalent commands for the terminal in Windows and macOS.

## A Short Tutorial

{{< hint warning >}}
**Windows Users**  
This tutorial includes some instruction on using the command line text editor `vim`. Windows doesn't have `vim` preinstalled so to follow along you'll have to install it yourself. Instructions for doing so are available in the [Helper](../helpers/vim) section of this site.
{{< /hint >}}
...
{{< hint danger >}}
**Warning**  
When you delete things on the terminal, be certain you won’t ever need them again.
{{< /hint >}}
...
<div style="display: flex; justify-content: space-between;">
    <div>
        {{< button relref="chapter-1" >}}Previous{{< /button >}}
    </div>
    <div>
        {{< button relref="chapter-3" >}}Next{{< /button >}}
    </div>
</div>